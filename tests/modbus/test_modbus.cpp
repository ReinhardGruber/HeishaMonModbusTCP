...
int main() {
  ...
  assert(count == NUMBER_OF_TOPICS + NUMBER_OF_TOPICS_EXTRA + NUMBER_OF_OPT_TOPICS +
                  arraySize(commands) + arraySize(optionalCommands) + 3);
  std::puts("PASS: complete map, expansion, scaling, command dispatch, coils, boundaries and register page");
}
